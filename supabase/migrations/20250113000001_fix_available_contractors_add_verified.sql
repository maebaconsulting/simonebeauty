-- Migration: 20250113000001_fix_available_contractors_add_verified.sql
-- Description: Add is_verified check to get_available_contractors function
-- This ensures only verified contractors appear in availability search
...
BEGIN
    ...
    WITH contractor_candidates AS (
        ...
        WHERE cs.service_id = p_service_id
          ...
          AND c.is_verified = true  -- ADDED: Only verified contractors
          AND cos.is_completed = true
    ),
    ...
    ORDER BY cst.booking_count DESC;
END;
$$;
...
FIXED: Added is_verified check to only show verified contractors.
