 ...
 *
 * Admin page for editing existing promo codes
 */
...
export default function EditPromoCodePage() {
  ...
  const { data: promoCode, isLoading: isLoadingPromo } = usePromoCode(
    isNaN(promoId) ? null : promoId
  )
  ...
  const handleSubmit = async (data: PromoCodeFormData) => {
    try {
      ...
      toast.success('Code promo modifié', {
        description: `Le code "${data.code}" a été mis à jour avec succès.`,
      })
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Code: <span className="font-mono font-bold">{promoCode.code}</span>
            </p>
          </div>
        </div>
      </div>
